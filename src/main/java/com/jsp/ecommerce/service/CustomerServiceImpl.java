...
public class CustomerServiceImpl implements CustomerService {
	...
	public String loadHome(HttpSession session, Model model) {
		...
		if (customer != null) {
			...
			if (cart != null) {
				cartCount = itemRepository.findByCart(cart).size();
			}

			model.addAttribute("customerName", customer.getName());
			model.addAttribute("cartCount", cartCount);
			model.addAttribute("orderCount", orders.size());
			model.addAttribute("recentOrders", orders.stream().sorted((o1, o2) -> o2.getCreationTime().compareTo(o1.getCreationTime())).limit(3).toList());
	...
	public String addToCart(Long id, HttpSession session) {
		...
		if (customer != null) {
			if (id == null) {
				session.setAttribute("fail", "Invalid Product Id");
	...
	public String increaseQuantity(Long id, HttpSession session) {
		...
		if (customer != null) {
			if (id == null) {
				session.setAttribute("fail", "Invalid Item Id");
